...
const ColorSelector = ({ selectedColor, onSelectColor }) => {
  const colores = [
    { name: 'Rojo', value: 'from-red-600 to-red-800', class: 'from-red-600 to-red-800' },
    { name: 'Amarillo', value: 'from-yellow-500 to-yellow-700', class: 'from-yellow-500 to-yellow-700' },
    { name: 'Verde', value: 'from-green-500 to-green-700', class: 'from-green-500 to-green-700' },
    { name: 'Azul', value: 'from-blue-500 to-blue-700', class: 'from-blue-500 to-blue-700' },
    { name: 'Púrpura', value: 'from-purple-600 to-purple-800', class: 'from-purple-600 to-purple-800' },
    { name: 'Rosa', value: 'from-pink-500 to-pink-700', class: 'from-pink-500 to-pink-700' },
    { name: 'Índigo', value: 'from-indigo-500 to-indigo-700', class: 'from-indigo-500 to-indigo-700' },
    { name: 'Naranja', value: 'from-orange-500 to-orange-700', class: 'from-orange-500 to-orange-700' },
    { name: 'Gris', value: 'from-gray-500 to-gray-700', class: 'from-gray-500 to-gray-700' },
  ];
      ...
      <div className="flex flex-wrap gap-2">
        {colores.map((color) => (
          <button
            ...
            className={`w-8 h-8 rounded-lg bg-gradient-to-br ${color.class} ${
              selectedColor === color.value ? 'ring-2 ring-offset-2 ring-red-600 scale-110' : ''
...
const RolModal = ({ isOpen, onClose, rol, onGuardar, modo }) => {
  ...
  const [color, setColor] = useState(rol?.color || 'from-purple-600 to-purple-800');
  ...
  useEffect(() => {
    if (rol) {
      ...
      setColor(rol.color || 'from-purple-600 to-purple-800');
    } else {
      setNombre('');
      setDescripcion('');
      setColor('from-purple-600 to-purple-800');
    }
  }, [rol, isOpen]);
...
const PermisosModal = ({ isOpen, onClose, rol, permisos, onGuardar, modulos, totalAcciones }) => {
  ...
  const [filtroModulos, setFiltroModulos] = useState([]);
  const { theme } = useTheme();
  ...
  useEffect(() => {
    if (busqueda.trim() === '') {
      ...
      const filtrados = modulos.filter(modulo =>
        modulo.nombre.toLowerCase().includes(busqueda.toLowerCase()) ||
        modulo.id.toLowerCase().includes(busqueda.toLowerCase())
      );
      setFiltroModulos(filtrados);
    }
  }, [busqueda, modulos]);
              ...
              {filtroModulos.length === 0 ? (
                <div className="text-center py-12">
                  <p className={`text-lg ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                    No se encontraron módulos que coincidan con "{busqueda}"
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {filtroModulos.map(modulo => {
        ...
        rolesSnap.forEach(doc => {
          ...
          if (!ROLES_FIJOS.find(r => r.id === rolId)) {
            ...
            rolesCargados.push({
              ...
              color: data.color || 'from-purple-600 to-purple-800',
              icon: CpuChipIcon,
              ...data
            });
          }
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center space-x-3 sm:space-x-4 w-full sm:w-auto">
          ...
          <div className="flex-1">
            ...
            <p className={`text-xs sm:text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
              Gestiona los roles y sus accesos a los módulos del sistema
            </p>
          </div>
        </div>
